...
<template>
  <div class="overflow-hidden rounded-2xl border border-stroke bg-surface/80">
    <table class="w-full text-left text-sm">
      ...
      <tbody>
        ...
        <tr v-else-if="!items.length">
          <td colspan="5" class="px-4 py-10 text-center text-muted">Nenhum lead encontrado.</td>
        </tr>
        <tr
          v-for="lead in items"
          v-else
    ...
    >
      ...
      <div class="flex items-center gap-2">
        <button
          type="button"
          class="inline-flex h-8 w-8 items-center justify-center rounded-full border border-stroke text-muted disabled:opacity-40"
          :disabled="page <= 1"
          @click="changePage(-1)"
        >
