...
async function load() {
  ...
  error.value = null
  try {
    const url: string = `/api/dashboard/leads/${props.leadId}`
    detail.value = (await $fetch(url)) as DashboardLeadDetailResponse
  } catch (e: unknown) {
...
async function addNote() {
  ...
  error.value = null
  try {
    const url: string = `/api/dashboard/leads/${lead.value.id}/notes`
    const note = (await $fetch(url, {
      method: 'POST',
      body: { body: draftNote.value.trim() }
    })) as LeadNote
    if (detail.value) {
...
async function removeLead() {
  ...
  try {
    const url: string = `/api/dashboard/leads/${lead.value.id}`
