...
async function save() {
  ...
  try {
    ...
    const updated = await $fetch<Task>(url as string, {
      method: 'PATCH',
      body: {
...
<template>
  ...
  >
    <aside
      v-if="draft"
      class="fixed inset-y-0 right-0 z-40 flex w-full max-w-md flex-col border-l border-stroke/80 bg-bg/95 shadow-[0_50px_120px_-40px_rgba(41,67,101,0.7)] backdrop-blur"
    >
      <header class="flex items-center justify-between border-b border-stroke/70 p-5">
        <div>
          ...
          <h3 class="font-display text-xl italic text-text-primary">{{ draft.title || 'Detalhes' }}</h3>
        </div>
        <button
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-full border border-stroke bg-bg/70 text-muted transition-colors hover:border-accent hover:text-text-primary"
          aria-label="Fechar"
          @click="emit('close')"
        >
      ...
      <section class="flex-1 space-y-4 overflow-y-auto p-5 text-sm">
        <label class="block space-y-1">
          ...
          <input
            v-model="draft.title"
            :disabled="!canEdit"
            class="w-full rounded-2xl border border-stroke bg-surface/85 px-3 py-2 text-text-primary outline-none transition-colors focus:border-accent"
          />
        </label>
        ...
        <label class="block space-y-1">
          ...
          <textarea
            ...
            :disabled="!canEdit"
            rows="4"
            class="w-full resize-none rounded-2xl border border-stroke bg-surface/85 px-3 py-2 text-text-primary outline-none transition-colors focus:border-accent"
          />
        </label>
      ...
      <footer class="flex items-center justify-end gap-2 border-t border-stroke/60 p-5">
        <button
          type="button"
          class="rounded-full border border-stroke bg-bg/70 px-4 py-1.5 text-xs uppercase tracking-[0.18em] text-muted transition-colors hover:border-accent hover:text-text-primary"
          @click="emit('close')"
        >
          Cancelar
        </button>
        <button
          type="button"
          class="gradient-border rounded-full border border-stroke bg-surface px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-text-primary transition-colors hover:bg-bg disabled:opacity-40"
